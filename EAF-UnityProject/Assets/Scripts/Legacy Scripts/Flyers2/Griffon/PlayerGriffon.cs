...
public class PlayerGriffon : FlyerPlayer
{
    ...
    protected override void FixedUpdate()
    {
        ...
        if (!needFlyToCenter)
        {
            ...
            if ((CrossPlatformInputManager.GetButton("StrafeRight") || Input.GetKey(KeyCode.Period)) && FinalSpeed<6f)
            {
                rb.velocity = transform.up * FinalSpeed + transform.right * StrafeSpeed;
            } else if ((CrossPlatformInputManager.GetButton("StrafeLeft") || Input.GetKey(KeyCode.Comma)) && FinalSpeed < 6f)
            {
                rb.velocity = transform.up * FinalSpeed + transform.right * -StrafeSpeed;
            }
            else {
    ...
    public void UseMainWeapon()
    {
        ...
        Instantiate(MuzzlePR, AttackPoint.position, AttackPoint.rotation);
